...
@observer
 class JBSwitch extends JBSwitchService{
    ...
    render(){
        var renderDom = <div className="jb-switch-component-wrapper" ref={(dom)=>{this.JBSwitchComponentDom = dom}}>
            <div className={"caption "+  (this.config.value?'active':'')}>{this.config.trueTitle?this.config.trueTitle:''}</div>
            <div className="svg-wrapper" onClick={(e)=>this.onClick(e)}>
                <svg className={"switch-svg " + (this.config.value?'active':'')} viewBox="0 0 710 440" >
                    <defs>      
                    ...
                    <circle className="trigger-circle-bar" cx="220" cy="220" r="120" filter="url(#dropshadow)"/>
                </svg>
            </div>
            <div className={"caption "+  (!this.config.value?'active':'')}>{this.config.falseTitle?this.config.falseTitle:''}</div>
        </div>
        return(renderDom);
